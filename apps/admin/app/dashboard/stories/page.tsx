...
export default async function StoriesPage({
  ...
  const { items: stories } = await pb.collection('stories').getList(1, 200, {
    filter,
    sort: 'display_order',
    fields: 'id,title,category,age_group,is_premium,status,display_order,updated',
  })
              ...
              {stories.map((story) => (
                <tr key={story.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600 capitalize">
                    {(story.category as string).replace('_', ' ')}
                  </td>
                  <td className="px-4 py-3 text-gray-600 capitalize">
                    {(story.age_group as string).replace('_', ' ')}
                  </td>
                  <td className="px-4 py-3">
                    <ContentStatusBadge status={story.status as ContentStatus} />
                  </td>
                  <td className="px-4 py-3 text-gray-400">
                    {new Date(story.updated as string).toLocaleDateString('en-IN')}
                  </td>
                  <td className="px-4 py-3">
                    <Link
